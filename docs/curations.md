...
1. `package_modifier`: "insert"
2. `package_modifier`: "delete"
...
3. Example "delete": every license in the given `file_scope` with the name "Apache-1.1" and a `license_text_in_archive` with a concrete path or null will be deleted
4. Example "update": "null" will be defined for the `license_text_in_archive` for licenses with the name "MIT" in the given `file_scope`
...
3. Example: "delete": the `copyright` for a "delete" may contain the wildcards "\*" and/or "?". In this example every copyright statement containing the text "2001" will be deleted.